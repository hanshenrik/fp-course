...
-- headOr defaultValue list = foldRight const defaultValue list
...
-- product numbers = foldRight (*) 1 numbers
product = foldRight (*) 1
...
sum :: List Int -> Int
-- sum numbers = foldRight (+) 0 numbers
sum = foldRight (+) 0
...
find ::
  ...
  Optional a
find p =
  foldRight (\a oa -> if p a then Full a else oa) Empty
